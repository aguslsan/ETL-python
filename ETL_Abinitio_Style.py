...
def execute_filters(transformations):
    for filter in transformations["filters"]:
        """
        # globals()[filter["variable_name"]] =
        # print(globals()[filter["variable_name"]])
        contador=0
        for registro in globals()[filter["variable_name"]]:
            if not eval(filter["code"]):
                print("true")
                # del globals()[filter["variable_name"]][contador]
                globals()[filter["variable_name"]].remove(registro)
            print("false")
            contador = contador + 1
        """
        print(type(globals()[filter["variable_name"]]))
        print(globals()[filter["variable_name"]])
        globals()[filter["variable_name"]] = list(filter(lambda x: eval(filter["code"]), globals()[filter["variable_name"]]))
        print(globals()[filter["variable_name"]])
